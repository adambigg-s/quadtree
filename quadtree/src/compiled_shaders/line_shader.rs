#![allow(dead_code)]
use sokol::gfx as sg;
/*
    #version:1# (machine generated, don't edit!)

    Generated by sokol-shdc (https://github.com/floooh/sokol-tools)

    Cmdline:
        sokol-shdc -i ./src/shaders/line_shader.glsl -o ./src/compiled_shaders/line_shader.rs --slang hlsl5:wgsl:glsl430 -f sokol_rust

    Overview:
    =========
    Shader program: 'line':
        Get shader desc: line_shader_desc(sg::query_backend());
        Vertex Shader: line_vs
        Fragment Shader: line_fs
        Attributes:
            ATTR_LINE_V_POS => 0
            ATTR_LINE_V_COLOR => 1
    Bindings:
        Uniform block 'v_params_world':
            Rust struct: VParamsWorld
            Bind slot: UB_V_PARAMS_WORLD => 0
*/
pub const ATTR_LINE_V_POS: usize = 0;
pub const ATTR_LINE_V_COLOR: usize = 1;
pub const UB_V_PARAMS_WORLD: usize = 0;
#[repr(C, align(16))]
pub struct VParamsWorld {
    pub world_dims: [f32; 2],
    pub _pad_8: [u8; 8],
}
/*
    #version 430

    uniform vec4 v_params_world[1];
    layout(location = 0) out vec3 f_color;
    layout(location = 1) in vec3 v_color;
    layout(location = 0) in vec2 v_pos;

    void main()
    {
        f_color = v_color;
        gl_Position = vec4(fma(v_pos.x / v_params_world[0].x, 2.0, -1.0), fma(v_pos.y / v_params_world[0].y, 2.0, -1.0), 1.0, 1.0);
    }

*/
pub const LINE_VS_SOURCE_GLSL430: [u8; 329] = [
    ,
];
/*
    #version 430

    layout(location = 0) out vec3 color;
    layout(location = 0) in vec3 f_color;

    void main()
    {
        color = f_color;
    }

*/
pub const LINE_FS_SOURCE_GLSL430: [u8; 129] = [
    ,
];
/*
    cbuffer v_params_world : register(b0)
    {
        float2 _25_world_dims : packoffset(c0);
    };


    static float4 gl_Position;
    static float3 f_color;
    static float3 v_color;
    static float2 v_pos;

    struct SPIRV_Cross_Input
    {
        float2 v_pos : TEXCOORD0;
        float3 v_color : TEXCOORD1;
    };

    struct SPIRV_Cross_Output
    {
        float3 f_color : TEXCOORD0;
        float4 gl_Position : SV_Position;
    };

    void vert_main()
    {
        f_color = v_color;
        gl_Position = float4(mad(v_pos.x / _25_world_dims.x, 2.0f, -1.0f), mad(v_pos.y / _25_world_dims.y, 2.0f, -1.0f), 1.0f, 1.0f);
    }

    SPIRV_Cross_Output main(SPIRV_Cross_Input stage_input)
    {
        v_color = stage_input.v_color;
        v_pos = stage_input.v_pos;
        vert_main();
        SPIRV_Cross_Output stage_output;
        stage_output.gl_Position = gl_Position;
        stage_output.f_color = f_color;
        return stage_output;
    }
*/
pub const LINE_VS_SOURCE_HLSL5: [u8; 839] = [
    ,
];
/*
    static float3 color;
    static float3 f_color;

    struct SPIRV_Cross_Input
    {
        float3 f_color : TEXCOORD0;
    };

    struct SPIRV_Cross_Output
    {
        float3 color : SV_Target0;
    };

    void frag_main()
    {
        color = f_color;
    }

    SPIRV_Cross_Output main(SPIRV_Cross_Input stage_input)
    {
        f_color = stage_input.f_color;
        frag_main();
        SPIRV_Cross_Output stage_output;
        stage_output.color = color;
        return stage_output;
    }
*/
pub const LINE_FS_SOURCE_HLSL5: [u8; 420] = [
    ,
];
/*
    diagnostic(off, derivative_uniformity);

    struct v_params_world {
      /_ @offset(0) _/
      world_dims : vec2f,
    }

    var<private> f_color : vec3f;

    var<private> v_color : vec3f;

    var<private> v_pos : vec2f;

    @group(0) @binding(0) var<uniform> x_25 : v_params_world;

    var<private> gl_Position : vec4f;

    fn main_1() {
      var ndcx : f32;
      var ndcy : f32;
      var ndc : vec2f;
      let x_12 : vec3f = v_color;
      f_color = x_12;
      let x_22 : f32 = v_pos.x;
      let x_30 : f32 = x_25.world_dims.x;
      ndcx = (((x_22 / x_30) * 2.0f) - 1.0f);
      let x_39 : f32 = v_pos.y;
      let x_41 : f32 = x_25.world_dims.y;
      ndcy = (((x_39 / x_41) * 2.0f) - 1.0f);
      let x_47 : f32 = ndcx;
      let x_48 : f32 = ndcy;
      ndc = vec2f(x_47, x_48);
      let x_55 : vec2f = ndc;
      gl_Position = vec4f(x_55.x, x_55.y, 1.0f, 1.0f);
      return;
    }

    struct main_out {
      @location(0)
      f_color_1 : vec3f,
      @builtin(position)
      gl_Position : vec4f,
    }

    @vertex
    fn main(@location(1) v_color_param : vec3f, @location(0) v_pos_param : vec2f) -> main_out {
      v_color = v_color_param;
      v_pos = v_pos_param;
      main_1();
      return main_out(f_color, gl_Position);
    }

*/
pub const LINE_VS_SOURCE_WGSL: [u8; 1102] = [
    ,
];
/*
    diagnostic(off, derivative_uniformity);

    var<private> color : vec3f;

    var<private> f_color : vec3f;

    fn main_1() {
      let x_12 : vec3f = f_color;
      color = x_12;
      return;
    }

    struct main_out {
      @location(0)
      color_1 : vec3f,
    }

    @fragment
    fn main(@location(0) f_color_param : vec3f) -> main_out {
      f_color = f_color_param;
      main_1();
      return main_out(color);
    }

*/
pub const LINE_FS_SOURCE_WGSL: [u8; 366] = [
    ,
];
pub fn line_shader_desc(backend: sg::Backend) -> sg::ShaderDesc {
    let mut desc = sg::ShaderDesc::new();
    desc.label = c"line_shader".as_ptr();
    match backend {
        sg::Backend::Glcore => {
            desc.vertex_func.source = &LINE_VS_SOURCE_GLSL430 as *const _ as *const _;
            desc.vertex_func.entry = c"main".as_ptr();
            desc.fragment_func.source = &LINE_FS_SOURCE_GLSL430 as *const _ as *const _;
            desc.fragment_func.entry = c"main".as_ptr();
            desc.attrs[0].glsl_name = c"v_pos".as_ptr();
            desc.attrs[1].glsl_name = c"v_color".as_ptr();
            desc.uniform_blocks[0].stage = sg::ShaderStage::Vertex;
            desc.uniform_blocks[0].layout = sg::UniformLayout::Std140;
            desc.uniform_blocks[0].size = 16;
            desc.uniform_blocks[0].glsl_uniforms[0]._type = sg::UniformType::Float4;
            desc.uniform_blocks[0].glsl_uniforms[0].array_count = 1;
            desc.uniform_blocks[0].glsl_uniforms[0].glsl_name = c"v_params_world".as_ptr();
        },
        sg::Backend::D3d11 => {
            desc.vertex_func.source = &LINE_VS_SOURCE_HLSL5 as *const _ as *const _;
            desc.vertex_func.d3d11_target = c"vs_5_0".as_ptr();
            desc.vertex_func.entry = c"main".as_ptr();
            desc.fragment_func.source = &LINE_FS_SOURCE_HLSL5 as *const _ as *const _;
            desc.fragment_func.d3d11_target = c"ps_5_0".as_ptr();
            desc.fragment_func.entry = c"main".as_ptr();
            desc.attrs[0].hlsl_sem_name = c"TEXCOORD".as_ptr();
            desc.attrs[0].hlsl_sem_index = 0;
            desc.attrs[1].hlsl_sem_name = c"TEXCOORD".as_ptr();
            desc.attrs[1].hlsl_sem_index = 1;
            desc.uniform_blocks[0].stage = sg::ShaderStage::Vertex;
            desc.uniform_blocks[0].layout = sg::UniformLayout::Std140;
            desc.uniform_blocks[0].size = 16;
            desc.uniform_blocks[0].hlsl_register_b_n = 0;
        },
        sg::Backend::Wgpu => {
            desc.vertex_func.source = &LINE_VS_SOURCE_WGSL as *const _ as *const _;
            desc.vertex_func.entry = c"main".as_ptr();
            desc.fragment_func.source = &LINE_FS_SOURCE_WGSL as *const _ as *const _;
            desc.fragment_func.entry = c"main".as_ptr();
            desc.uniform_blocks[0].stage = sg::ShaderStage::Vertex;
            desc.uniform_blocks[0].layout = sg::UniformLayout::Std140;
            desc.uniform_blocks[0].size = 16;
            desc.uniform_blocks[0].wgsl_group0_binding_n = 0;
        },
        _ => {},
    }
    desc
}
